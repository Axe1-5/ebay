...
public class GetBiersServices extends IntentService {
    ...
    private static final String ACTION_get_all_biers = "org.esiea.patel_verrier.ebay.action.get_all_biers";
    ...
    private void handleActionget_all_biers() {
        ...
        URL url=null;
        try {
            url=new URL("https://pokeapi.co/api/v1/pokemon/?limit=250");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
            conn.connect();
            if(HttpURLConnection.HTTP_OK == conn.getResponseCode()) {
                copyInputStreamToFile(conn.getInputStream(),
                        new File(getCacheDir(),"pokemon.json"));
                Log.d("tag","Pokemon downloaded");
